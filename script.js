...
const Calculator = {
  ...
  getUserMeasurement: () => {
    ...
    const measuredType = document.querySelector(
      'input[name="measuredType"]:checked'
    )?.value;
...
const UI = {
  ...
  displayResults: (normative, userMeasurement, age, heightCm) => {
    ...
    document.getElementById("femaleIndex").textContent =
      normative.female.index.toFixed(2);
    document.getElementById("femaleArea").textContent =
      normative.female.area.toFixed(2);
    document.getElementById("femaleDiameter").textContent =
      normative.female.diameter.toFixed(2);

    document.getElementById("maleIndex").textContent =
      normative.male.index.toFixed(2);
    document.getElementById("maleArea").textContent =
      normative.male.area.toFixed(2);
    document.getElementById("maleDiameter").textContent =
      normative.male.diameter.toFixed(2);

    document.getElementById("results").style.display = "block";

    // Display user's measurement comparison if provided
    if (userMeasurement) {
      document.getElementById("userDiameter").textContent =
        userMeasurement.diameter.toFixed(2);
      document.getElementById("userArea").textContent =
        userMeasurement.area.toFixed(2);
      document.getElementById("userIndex").textContent =
        userMeasurement.index.toFixed(2);

      // Female comparison
      const femalePercentDiff =
        ((userMeasurement.index - normative.female.index) /
          normative.female.index) *
        100;

      let femaleText = "";
      if (femalePercentDiff > 0) {
        femaleText = `${femalePercentDiff.toFixed(
          1
        )}% above female upper limit`;
      } else if (femalePercentDiff < 0) {
        femaleText = `${Math.abs(femalePercentDiff).toFixed(
          1
        )}% below female upper limit`;
      } else {
      ...
      // Male comparison
      const malePercentDiff =
        ((userMeasurement.index - normative.male.index) /
          normative.male.index) *
        100;
      ...
      if (malePercentDiff > 0) {
        ...
        maleText = `${Math.abs(malePercentDiff).toFixed(
          1
        )}% below male upper limit`;
      } else {
...
const ChartManager = {
  ...
  createChart: (age, userHeightCm, normative, userMeasurement) => {
    ...
    if (userHeightCm) {
      const femaleValueAtHeight = ChartManager.calculateValueAtHeight(
        age,
        true,
        userHeightCm,
        chartType
      );
      const maleValueAtHeight = ChartManager.calculateValueAtHeight(
        age,
        false,
        userHeightCm,
        chartType
    ...
    if (userMeasurement && userHeightCm) {
      const userY =
        chartType === "area" ? userMeasurement.area : userMeasurement.diameter;
      datasets.push({
      ...
      options: {
        ...
        plugins: {
          legend: {
            ...
            position: "top",
            labels: {
              boxWidth: 12, // Smaller legend boxes on mobile
              padding: 10,
              font: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const heightUnitRadios = document.querySelectorAll(
    'input[name="heightUnit"]'
  );
  const measuredValueInput = document.getElementById("measuredValue");
  const measuredTypeRadios = document.querySelectorAll(
    'input[name="measuredType"]'
  );
  ...
  chartTypeRadios.forEach((radio) => {
    radio.addEventListener("change", () => {
      if (ChartManager.chart) {
        ...
        const heightMeters = Conversions.getHeightInMeters();
        const normative = Calculator.calculateNormativeValues(
          age,
          heightMeters
        );
